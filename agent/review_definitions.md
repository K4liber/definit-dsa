...
3. Check if a definition content contains an example that illustrates the definition. If does, please place it under a new segment under the content, and split it with a `---` separator. If it does not, please create an example that illustrates the definition. Please use references in the example. The example should be simple and does not contain references to higher-level definitions. It also does not need to contain all references that the definition have. Just those one that are needed to illustrate the definition.
