...
class _InPlaceAlgorithm(Definition):
    def _get_content(self) -> str:
        return f"""
An {ALGORITHM.key.get_reference(phrase="algorithm")} that transforms a 
{DATA_STRUCTURE.key.get_reference(phrase="data structure")} using only a constant amount of 
{AUXILIARY_SPACE.key.get_reference(phrase="auxiliary space")}, meaning it 
{OPERATION.key.get_reference(phrase="operates")} directly on the {INPUT_DATA.key.get_reference(phrase="input data")} 
rather than creating a separate copy. An in-place algorithm typically has a 
{SPACE_COMPLEXITY.key.get_reference(phrase="space complexity")} of {BIG_O_NOTATION.key.get_reference(phrase="O(1)")}, 
