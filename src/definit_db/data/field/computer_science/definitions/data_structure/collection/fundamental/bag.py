...
class _Bag(Definition):
    def _get_content(self) -> str:
        return f"""
A bag is a {COLLECTION.key.get_reference(phrase="collection")} that allows for the 
{STORAGE.key.get_reference(phrase="storage")} of multiple {ITEM.key.get_reference(phrase="items")}, where the same 
item can be stored multiple times. It realizes the {MULTISET.key.get_reference(phrase="multiset")} abstraction as an 
{ABSTRACT_DATA_TYPE.key.get_reference(phrase="abstract data type")}, but focuses on the 
{OPERATION.key.get_reference(phrase="operations")} of adding items and 
...
multiplicity is preserved. A {PROGRAM.key.get_reference(phrase="program")} can add more scores and walk through 
every stored item, but it cannot remove a specific one.
"""
