...
- Push, which adds an {ITEM.key.get_reference(phrase="element")} to the collection, and

- Pop, which removes the most recently added {ITEM.key.get_reference(phrase="element")}.

Additionally, a peek {OPERATION.key.get_reference(phrase="operation")} can, without modifying the stack, return 
the value of the last {ITEM.key.get_reference(phrase="element")} added. The name stack is an analogy to a 
{SET.key.get_reference(phrase="set")} of physical items stacked one atop another, such as a stack of plates.

---

Starting empty, pushing A, then B, then C places C on top. The first pop removes C, the most recently added 
{ITEM.key.get_reference(phrase="element")}; the next removes B; the last removes A. 
The {ITEM.key.get_reference(phrase="element")} added last is always the one removed next.
"""
