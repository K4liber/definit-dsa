...
class _Array(Definition):
    def _get_content(self) -> str:
        return f"""
Array is a {LIST.key.get_reference(phrase="list")} of {ITEM.key.get_reference(phrase="elements")} of the same 
{DATA_TYPE.key.get_reference(phrase="type")}, stored contiguously so that each 
{ITEM.key.get_reference(phrase="element")} can be accessed directly by its {INDEX.key.get_reference(phrase="index")}.
...
matter where it sits, because the index directly determines the {ITEM.key.get_reference(phrase="element")}'s position.
"""
