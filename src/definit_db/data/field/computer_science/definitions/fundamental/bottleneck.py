...
class _Bottleneck(Definition):
    def _get_content(self) -> str:
        return f"""
A part of a {SOFTWARE_SYSTEM.key.get_reference("system")} that limits overall
...
An algorithm first applies {SORTING.key.get_reference()} to a list of n elements in
{BIG_O_NOTATION.key.get_reference(phrase="O(n log n)")} time, then scans it once in
{BIG_O_NOTATION.key.get_reference(phrase="O(n)")} time. The {SORTING.key.get_reference("sorting")} step is
the bottleneck, since its {COMPLEXITY.key.get_reference("complexity")} grows faster than the scanning step as n
