...
class _DeepCopy(Definition):
    def _get_content(self) -> str:
        return f"""
An {OPERATION.key.get_reference(phrase="operation")} that creates a new 
...
{MEMORY_ALLOCATION.key.get_reference(phrase="memory allocations")}, so changing nested 
{DATA.key.get_reference(phrase="data")} through one object does 
not change the other object through shared {POINTER.key.get_reference(phrase="pointers")}.

---

Suppose object A holds a {LIST.key.get_reference(phrase="list")} that itself references two other objects, B and C. 
A deep copy of A produces a new object 
