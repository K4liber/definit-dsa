...
class _Lopsided(Definition):
    def _get_content(self) -> str:
        return f"""
A situation where something is unbalanced or heavily skewed toward one side.
...
side may contain a long chain of {NODE.key.get_reference(phrase="nodes")} while the other side is nearly empty, 
so the tree leans heavily toward one side rather than spreading its {NODE.key.get_reference(phrase="nodes")} evenly.
"""
