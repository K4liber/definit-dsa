...
class _OperatingSystem(Definition):
    def _get_content(self) -> str:
        return f"""
An operating {SOFTWARE_SYSTEM.key.get_reference(phrase="system")} that manages a 
{COMPUTER.key.get_reference(phrase="computer")}'s
...
how much memory each receives, and how they take turns reading from the {DISK.key.get_reference(phrase="disk")}. 
A program never touches the hardware directly; instead it asks the operating system, which mediates access so the 
