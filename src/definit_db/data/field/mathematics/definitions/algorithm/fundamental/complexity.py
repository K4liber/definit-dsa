...
{PROBLEM.key.get_reference()}, typically expressed as a {FUNCTION.key.get_reference()} of the 
{INPUT_DATA.key.get_reference(phrase="input")} size. Complexity quantifies how the resource requirements grow as the 
input size increases, helping to evaluate and compare {ALGORITHM.key.get_reference()} efficiency.
