...
Complexity theory classifies {PROBLEM.key.get_reference(phrase="problems")} like "can this 
{BOOLEAN_EXPRESSION.key.get_reference(phrase="Boolean")} formula be satisfied?" according to whether they can be solved 
{EFFICIENCY.key.get_reference(phrase="efficiently")}, whether proposed answers can be checked efficiently, 
and how those classes are connected by {RELATION.key.get_reference(phrase="relationships")} of difficulty. 
A central question in the area is whether every problem whose answer can be checked quickly can also be solved quickly.
"""
