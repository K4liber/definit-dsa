...
calls and returns the stored result when the same inputs occur again. {self.key.get_reference()} is particularly 
...
{self.key.get_reference("Memoization")} stores each "Fn" the first time it is computed, so later calls with the 
