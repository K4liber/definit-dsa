...
class _BinarySearch(Definition):
    def _get_content(self) -> str:
        return f"""
Binary Search is an {ALGORITHM.key.get_reference()} that finds the position of a target value within a 
{SEQUENCE.key.get_reference("sequence")} by repeatedly dividing the search {INTERVAL.key.get_reference()} in half. 
Binary search requires the {SEQUENCE.key.get_reference("sequence")} to be {SORTING.key.get_reference(phrase="sorted")}. 
At each step it compares the target with the middle {ITEM.key.get_reference("element")} of the interval and then 
continues the search on the left or right half, effectively using a {DIVIDE_AND_CONQUER.key.get_reference()} approach.
