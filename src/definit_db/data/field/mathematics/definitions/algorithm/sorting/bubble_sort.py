...
{SEQUENCE.key.get_reference("sequence")}, compares {ADJACENT.key.get_reference(phrase="adjacent")} elements and swaps 
them if they are in the wrong order. This process is repeated until the entire {SEQUENCE.key.get_reference("sequence")} 
is sorted. Bubble sort is simple but generally inefficient for large inputs, as it repeatedly passes over the sequence.
