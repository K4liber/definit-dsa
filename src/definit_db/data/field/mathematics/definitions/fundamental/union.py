...
class _Union(Definition):
    def _get_content(self) -> str:
        return f"""
The {self.key.get_reference()} of two {SET.key.get_reference("sets")} A and B is the set of
{ITEM.key.get_reference("elements")} that are in A or in B (or in both).

---

A = (1, 2, 3) and B = (2, 3, 4).
The {self.key.get_reference()} of A and B is the {SET.key.get_reference("set")} (1, 2, 3, 4),
