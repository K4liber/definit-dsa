...
{REDUCTION.key.get_reference()} used in {COMPLEXITY_THEORY.key.get_reference()}. {PROBLEM.key.get_reference()} A 
...
{FUNCTION.key.get_reference()} that transforms any {INPUT_DATA.key.get_reference(phrase="instance")} of A into an 
equivalent instance of B — meaning A's answer is "yes" if and only if the transformed B instance's answer is "yes". 
This is written A ≤ₚ B and is interpreted as "A is no harder than B": an efficient solver for B can be used to solve 
A via the transformation.
...
Solve B -> Answer for A ← (same yes/no answer) ← Answer for B
...
{EFFICIENCY.key.get_reference(phrase="efficiently")} solvable, A would be too (apply f first, then solve B).
"""
